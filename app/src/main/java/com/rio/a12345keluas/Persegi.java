package com.rio.a12345keluas;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.TextView;

public class Persegi extends AppCompatActivity {
    private EditText inputSisi;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_persegi);

        inputSisi = findViewById(R.id.input_nilaiSisi);

        Button hitungButton = findViewById(R.id.hitung_persegi);
        hitungButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                hitungPersegi();
            }
        });
    }


    private void hitungPersegi() {
        String sisiStr = inputSisi.getText().toString();

        if (!sisiStr.isEmpty()) {
            double sisi = Double.parseDouble(sisiStr);

            double luas = sisi * sisi;
            double keliling = 4 * sisi;

            kirimHasilKeMainActivity(luas, keliling);
        }
    }

    private void kirimHasilKeMainActivity(double luas, double keliling) {
        Intent intent = new Intent();
        intent.putExtra("luas", luas);
        intent.putExtra("keliling", keliling);
        setResult(RESULT_OK, intent);
        finish();
    }
}